...
### 製品説明（具体的な製品の説明）
名刺や手帳など「人を忘れないようにするツール」はたくさん存在しますが、「忘れた人を思い出せるツール」は存在していません。

私たちは、**顔画像からその人の名前を逆引きするシステム**を実装する事で、自分の記憶に頼る事なく、人の名前を思い出せるツールを実現しました。

ユーザーは対象人物を専用のデバイスで撮影します。撮影された写真はすぐにサーバにアップロードされ、データベース上の顔写真と照合。最も近い人物の名前がユーザーに通知されます。
...
#### 0. Webサイトでの登録（前準備）
全てのユーザーは自分の顔画像をデータベースに登録する必要があります。  
専用のWebサイトでアカウントを作成し、顔写真を登録することで、データベースに追加されます。    
